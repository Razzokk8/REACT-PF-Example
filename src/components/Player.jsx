...
function Player() {
    ...
    function VolumeBtns(){
        return mute
            ? <VolumeOffIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={() => setMute(!mute)} />
            : volume <= 20 ? <VolumeMuteIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={() => setMute(!mute)} />
            : volume <= 75 ? <VolumeDownIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={() => setMute(!mute)} />
            : <VolumeUpIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={() => setMute(!mute)} />
    }
            ...
            <CustomPaper>
                <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
                    ...
                    <Stack direction='row' spacing={1}
                        ...
                        <FastRewindIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={toggleBackward}/>
                        {!isPlaying
                            ?   <PlayArrowIcon fontSize={'large'} sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={togglePlay}/>
                            :   <PauseIcon fontSize={'large'} sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={togglePlay}/>
                        }
                        <FastForwardIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={toggleForward} />
                        <SkipNextIcon sx={{color: '#0275d8', '&:hover': {color: 'white'}}} onClick={toggleSkipForward}/>
                    </Stack>
                    <Stack sx={{
                        display: 'flex',
                ...
                <div className='d-flex justify-content-center text-white'>{listName[index]}</div>
                <div className='d-flex justify-content-center'>
               <p><sm className='text-muted smallInfo'>{rightsList[index]}</sm></p></div>
            </CustomPaper>
        </Div>
